 ...
 */
LawList = Ext.extend(Ext.app.BaseFuncPanel, {
	initComponent: function() {
		Ext.apply(this, {
			gridConfig: {
				...
				storeMapping: [
					'id', 'title', 'content', 'created', 'applyMan', 'published', 'status', 'statusText', 'refuseReason'
				]
			},
			buttonConfig: ['all', '-', 
			...
			winConfig: {
				height: 470,
				width: 700,
				bigIconClass: 'lawBigIcon',
				desc: '新增，修改政策法规信息'
			},
			formConfig: {
				layout: 'column',
				items: [
					{xtype: 'fieldset', title: '政策法规信息', items: [
		...
		LawList.superclass.initComponent.call(this);
	...
	listWithSelected: function(combo, record, index) {
		Ext.getCmp('lawList').loadData({status: record.data.text});
	}
});

Law = Ext.extend(Ext.Panel, {
	layout: 'border',
	closable: true,
	initComponent: function() {
		this.center = new LawList({
			id: 'lawList',
		...
		Law.superclass.initComponent.call(this);
	},
	loadData: function() {
