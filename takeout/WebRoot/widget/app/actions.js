 ...
 */
ActionsList = Ext.extend(Ext.app.BaseFuncPanel, {
	initComponent: function() {
		Ext.apply(this, {
			...
			winConfig: {
				height: 470,
				width: 700,
				bigIconClass: 'actionsBigIcon',
				desc: '新增，修改文件通知信息'
			},
			formConfig: {
				layout: 'column',
				items: [
					{xtype: 'fieldset', title: '文件通知信息', items: [
		...
		ActionsList.superclass.initComponent.call(this);
						...
						handler: function() {
							...
							Ext.Ajax.request({
								...
								success: function(response, opts) {
									...
									if(jo.success) {
										win.close();
										Ext.getCmp('actionsList').loadData();
									}
								}
							});
						}
	...
	listWithSelected: function(combo, record, index) {
		Ext.getCmp('actionsList').loadData({status: record.data.text});
	}
});

Actions = Ext.extend(Ext.Panel, {
	layout: 'border',
	closable: true,
	initComponent: function() {
		this.center = new ActionsList({
			id: 'actionsList',
		...
		Actions.superclass.initComponent.call(this);
	},
	loadData: function() {
