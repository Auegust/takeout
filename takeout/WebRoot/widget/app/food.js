...
FoodList = Ext.extend(Ext.app.BaseFuncPanel, {
	initComponent: function() {
		Ext.apply(this, {
			gridConfig: {
				...
				defaults: {width: 200, align: 'center'},
					columns: [
						...
						{header: '食品名', dataIndex: 'name'},
						{header: '价格', dataIndex: 'price'},
						{header: '详情', dataIndex: 'description'}
					]
				}),	
				storeMapping: [
			...
			buttonConfig: ['all',
				...
				{xtype: 'f-search', emptyText: '请输入食品名'}
			],
			winConfig: {
				height: 470,
				width: 650,
				...
				items: [
					{xtype: 'fieldset', title: '文字信息', items: [
						{xtype: 'panel', layout: 'form', border: false, items: [
							{xtype: 'f-text', fieldLabel: '食品名', name: 'name', allowBlank: false},
							{xtype: 'f-text', fieldLabel: '价格', name: 'price'},
							{xtype: 'htmleditor', fieldLabel: '详情', name: 'content', height: 220, allowBlank: false}
						]}
					]}
				]
			},
	...
	listWithSelected: function(combo, record, index) {
		Ext.getCmp('foodList').loadData({status: record.data.text});
	}
});
