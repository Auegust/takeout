...
App = function() {
	...
	return [ '<table><tr><td><div class="round-box" >',
		'<div class="content ', msgType, '"><p align="center">', s,
		'</p></div>', '<div class="bottom">', '<div class="r4 ',
		msgType, '"/></div>', '<div class="r3 ', msgType, '"/></div>',
		'<div class="r2 ', msgType, '"/></div>', '<div class="r1 ',
		msgType, '"/></div>', '</div></div></td></tr></table>' ]
		.join('');
    }
    return {
	msg : function(s, error) {
		...
		html : createBox(s, error ? 'errorMsg' : 'infoMsg')
	    }, true);
	    msgCt.alignTo(document, 't-t', [ 0, -2 ]);
	    m.slideIn('t').pause(2).slideOut("t", {
		remove : true
	    });
	...
	.extend(
		Ext.Panel,
		{
			...
			if (item.fileType == "htm")
			    ...
			    window.location = ctx
				    + '/system/downloadManual?fileType='
				    + item.fileType;
...
App.MainPanel = Ext.extend(Ext.TabPanel, {
	...
	eval('var newTab = new ' + funcName + '({id:"' + funcName
		+ '",funcCode:"' + funcName + '"})');
